...
describe('NewQueryBuilder - INNER JOIN', () => {
  ...
  it('should generate INNER JOIN query', () => {
    const sql = queryBuilder
      ...
      .toString();
    expect(sql).toBe('SELECT * FROM user INNER JOIN post ON post.userId = 1');
  });
  ...
  it('should generate INNER JOIN with multiple ON conditions', () => {
    const sql = queryBuilder
      ...
      .toString();
    expect(sql).toBe("SELECT * FROM user INNER JOIN post ON post.userId = 1 AND post.title = 'João'");
  });
});
