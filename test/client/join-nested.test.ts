...
describe('NewQueryBuilder - Nested JOIN (join em join)', () => {
  ...
  it('should generate nested JOIN (join em join)', () => {
    const sql = queryBuilder
      ...
      .toString();
    expect(sql).toBe('SELECT * FROM post LEFT JOIN comment ON comment.postId = 1 INNER JOIN user ON user.id = 2');
  });
  ...
  it('should generate multiple joins at the same level', () => {
    const sql = queryBuilder
      ...
      .toString();
    expect(sql).toBe("SELECT * FROM post LEFT JOIN comment ON comment.postId = 1 INNER JOIN user ON user.id = 2 AND user.name = 'João'");
  });
