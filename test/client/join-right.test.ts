...
describe('NewQueryBuilder - RIGHT JOIN', () => {
  ...
  it('should generate RIGHT JOIN query', () => {
    const sql = queryBuilder
      ...
      .toString();
    expect(sql).toBe('SELECT * FROM user RIGHT JOIN post ON post.userId = 1');
  });
  ...
  it('should generate RIGHT JOIN with multiple ON conditions', () => {
    const sql = queryBuilder
      ...
      .toString();
    expect(sql).toBe("SELECT * FROM user RIGHT JOIN post ON post.userId = 1 AND post.title = 'João'");
  });
});
