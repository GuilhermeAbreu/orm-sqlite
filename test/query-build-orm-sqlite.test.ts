...
describe('QueryBuildOrmSQlite', () => {
    ...
    it('should generate a correct SELECT query with LEFT JOIN on JOIN', function () {
        ...
        const expectedQuery = "SELECT user.*, CASE WHEN posts.id IS NOT NULL THEN json_group_array( DISTINCT json_object( 'id', posts.id, 'comentarios', COALESCE( ( SELECT json_object( 'descricao', comentarios.descricao, 'id', comentarios.id, 'postId', comentarios.postId ) FROM comentarios as comentarios WHERE comentarios.postId = posts.id ), NULL ) , 'userId', posts.userId, 'title', posts.title ) ) ELSE NULL END AS posts FROM user LEFT JOIN post posts ON user.id = posts.userId ORDER BY user.name ASC LIMIT 10 OFFSET 5";
